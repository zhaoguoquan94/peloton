...
ProcessResult PostgresProtocolHandler::ExecQueryMessage(
  ...
  if (!query.empty()) {
    ...
    switch (query_type) {
      ...
      case QueryType::QUERY_EXECUTE: {
        ...
        if (param_values.size() > 0) {
          traffic_cop_->GetStatement()->GetPlanTree()->SetParameterValues(
              &param_values);
        }
      ...
      default: {
        ...
        traffic_cop_->SetStatement(traffic_cop_->PrepareStatement(
            unnamed_statement, traffic_cop_->query_, error_message));
    ...
    CompleteCommand(traffic_cop_->query_,
                    traffic_cop_->GetStatement()->GetQueryType(),
...
void PostgresProtocolHandler::ExecExecuteMessageGetResult(ResultType status) {
  ...
  switch (status) {
    ...
    default: {
      ...
      CompleteCommand(traffic_cop_->GetStatement()->GetQueryTypeString(),
                      query_type, traffic_cop_->getRowsAffected());
      return;
    }
  }
}
